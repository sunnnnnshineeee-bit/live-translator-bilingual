...
| 说好几句话才翻一句 / 译文落后好几句<br>Translations arrive several sentences late | 看后端终端的 `Whisper took Xs for Ys of audio` 日志：X 大于 Y 说明这台电脑实时识别跟不上。现在积压的切片会自动合并成一次识别（不再丢内容）、线程数自动匹配 CPU，延迟有上限。还慢就：关掉占 CPU 的程序；把 start-llama 里 `-np 2` 改成 `-np 1`；或从同一个 whisper.cpp 的 Hugging Face 仓库下载更小的 `ggml-tiny-q5_1.bin` 放进 `whisper.cpp/models/`，用 `$env:WHISPER_MODEL="whisper.cpp/models/ggml-tiny-q5_1.bin"` 启动后端<br>Watch the `Whisper took Xs for Ys of audio` line: if X > Y the PC can't transcribe in real time. Queued chunks are now merged into single Whisper calls (no content loss) and thread counts match the CPU automatically. Still slow? Close CPU-heavy apps, set `-np 1` in start-llama, or download the smaller `ggml-tiny-q5_1.bin` and set `$env:WHISPER_MODEL` accordingly |
